...
#
install_init(){
	# This will be displayed as your header from TMH0 to TMHX
	TMH1="Main install.sh"
	TMH2="*******************************"
	TMH3="      asusStockapps + Done"
	TMH4="*******************************"
	TMH5="app: $uFS_name"
...
install_post() {
	ui_print " "
	ui_print "POST-SCRIPT"
	
	local uFS_name="tweakPropB"   # addon name
	local uFS_src_ver=b1.0.4      # Source
	local uFS_rev_ver=08.22.2017  # Revision
	...
	tweakprop_me || {
		ui_print " - ERROR in Restoring Backup"
...
tweakprop_me() {
	...
	for bak in BACKUP= backup= Backup=;do
		(cat $tweak | grep -q -m 1 $bak) && answer=$(sed "s/$bak//p;d" "$tweak")
